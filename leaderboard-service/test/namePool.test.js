...
test("isValidWord only accepts exact, case-sensitive matches from the given list", () => {
  assert.equal(isValidWord("Rowdy", NAME_A), true);
  assert.equal(isValidWord("rowdy", NAME_A), false); // case-sensitive, mirrors PHP's in_array(..., true)
  assert.equal(isValidWord("Otter", NAME_A), false); // a NAME_B word, wrong list
...
test("isValidWord rejects rogue names — nothing outside the closed list is ever valid", () => {
  const rogueCandidates = [
    "",                          // empty
    " ",                         // whitespace only
    "Rowdy ",                    // trailing whitespace on an otherwise-real word
    " Rowdy",                    // leading whitespace
    "Rowdy\n",                   // trailing newline
    "Rowdy Otter",               // a full adjective+noun pair, not a single word
    "Rowdyotter",                // two real words mashed together
    "ROWDY",                     // wrong case
    "Rowdi",                     // near-miss typo on a real word
    "Tide-",                     // a fragment of the real "Tide-tossed" compound
    "'; DROP TABLE stampede_scores; --", // SQL-injection-shaped
    "<script>alert(1)</script>", // XSS-shaped
    "../../etc/passwd",          // path-traversal-shaped
    "Rowdy\u0000",                // embedded null byte after a real word
    "ℜ𝔬𝔴𝔡𝔶",                     // unicode-lookalike of "Rowdy", not the real bytes
  ];
...
test("a claim is only accepted when BOTH words come from their own list — a rogue word on either side fails the whole pair", () => {
  ...
  assert.equal(wouldClaim("Rowdy", "Otter"), true); // both real, real pair
  assert.equal(wouldClaim("Rowdy", "Nonexistent"), false); // rogue noun
  assert.equal(wouldClaim("Nonexistent", "Otter"), false); // rogue adjective
  assert.equal(wouldClaim("Otter", "Rowdy"), false); // real words, but swapped lists
