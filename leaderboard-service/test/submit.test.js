...
test("first submission saves the score", async () => {
  ...
  await seedClaim(redis, { token: "t1", name: "Salty Surfboard", createdAtSec: 1000 });
...
test("a lower second submission does not replace the score", async () => {
  ...
  const redis = new FakeRedis();
  await seedClaim(redis, { token: "t3", name: "Zippy Dolphin", score: 10000, createdAtSec: 1000 });
...
test("an equal second submission does not replace the score", async () => {
  const redis = new FakeRedis();
  await seedClaim(redis, { token: "t4", name: "Wild Lagoon", score: 10000, createdAtSec: 1000 });
...
test("a lower submission does not change the best-score timestamp", async () => {
  const redis = new FakeRedis();
  await seedClaim(redis, { token: "t5", name: "Sunny Stingray", score: 10000, createdAtSec: 1000 });
...
test("a lower submission does not change the leaderboard's ZSET composite", async () => {
  const redis = new FakeRedis();
  await seedClaim(redis, { token: "t7", name: "Feisty Pelican", score: 10000, createdAtSec: 1000 });
...
test("an equal submission never touches the composite", async () => {
  const redis = new FakeRedis();
  await seedClaim(redis, { token: "t9", name: "Gritty Seagull", score: 10000, createdAtSec: 1000 });
  const compositeBefore = 99990000;
...
test("a successful first submission removes the token from :unplayed", async () => {
  const redis = new FakeRedis();
  await seedClaim(redis, { token: "t10", name: "Plucky Seagull", createdAtSec: 1000 });
