#!/usr/bin/env node
// Measures tube registration (tw/cx/by, same shape as DUCK_REG/HURT_REG/DIE_REG)
// for the new cowabunga-sprites/die/die_0N.png frames, off the real PNG
...
// ring detection (this art is the same COWABUNGA-labelled teal tube badge
// used by idle/eat/move/duck, not the old typhoon rider's orange tube that
// die_01-03 used before).
//
// Also measures the reference idle frame (cowabunga-idle_02.png) with the
// same probe, so index.html can pin the die frames' tube width to IDLE's
// own on-screen tube size (IDLE_TUBE_W/CX/BY) instead of RIDER_TUBE_W (the
// old typhoon tube, unrelated to this badge art's proportions) -- same
// reasoning as DUCK_REG's own comment.
//
// node tools/die-frame-measure.js
    ...
    const result = await evaluate(session, `
      (async () => {
        ...
        for (const { key, src } of sources) {
          ...
          // Same reassemble-the-ring-from-its-arcs approach as
          // duck-frame-measure.js: the die poses' crossed/flailing arms and
          // the water-splash decorations both interrupt/mimic the ring, so
          // take the UNION of every connected teal component wide enough
          // (>35% of frame width) to plausibly be a ring arc, rather than
          // trusting the single largest component or a naive full-teal bbox.
