...
async function main() {
  ...
  try {
    ...
    const nameShownAfterTap = await evaluate(session, `byId("namePanel").classList.contains("on")`);
    ...
    const nameHiddenAfterSkip = await evaluate(session, `!byId("namePanel").classList.contains("on")`);
    ...
    const nameShownAfterArrowKey = await evaluate(session, `byId("namePanel").classList.contains("on")`);
